...
export function createSemaphore(limit = 2) {
  ...
  /** 执行已获名额的任务：跑 fn 并把结果交给 promise（成败都释放名额）。 */
  async function execTask(fn, resolve, reject) {
    state.active += 1;
    try {
      resolve(await fn());
    } catch (e) {
  ...
  /** 获取执行权：有空位立即跑，否则排队等名额释放。 */
  function acquire(fn, resolve, reject) {
    const task = () => { execTask(fn, resolve, reject).catch(reject); };
    if (state.active < limit) task();
    else state.queue.push(task);
  }
  const run = (fn) => new Promise((resolve, reject) => acquire(fn, resolve, reject));
  return { run };
}

/**
 * 释放一个并发名额并唤醒队首等待者（完成/失败都必须调，防名额泄漏）。
 * @param {{active: number, queue: Array<Function>}} state 信号量状态
 */
