// dsh-skill-scoreboard — 浏览器半侧（v1.3.0）
  ...
  factory: (require) => {
    ...
    const L = {
      zh: {
        ...
        'settings.desc': 'AI 会话中 skill 工具的使用次数自动统计（按会话去重，跨会话累加）。数据由宿主端 tools/result 监听写入 data/skill-usage.json。',
      ...
      en: {
        ...
        'settings.desc': 'Automatic skill usage counts across AI sessions (deduplicated per session, accumulated across sessions). Written by the host tools/result listener into data/skill-usage.json.',
    ...
    function ScoreboardPage() {
      ensureCss()
      const [state, setState] = React.useState({ loading: true, error: '', skills: [], total: 0, updatedAt: null, notice: '' })
      ...
      return React.createElement('div', { className: 'dshsb_page' },
        React.createElement('div', { className: 'dshsb_hero' },
          ...
          React.createElement('div', { className: 'dshsb_meta' },
            React.createElement('span', { className: 'dshsb_badge' }, tr('settings.totalBadge', { n: state.total })),
              ...
              : !state.loading && !rows.length
                ...
                : React.createElement('li', { className: 'dshsb_rows' },
                    rows.map((row) => React.createElement('div', {
                      ...
                      React.createElement('span', { className: 'dshsb_count' }, String(row.count)),
