...
//   Skill：skill 排行；二级翻页切换「按会话去重 / 按全部加载」两种规则；列表分页（« ‹ 页码 › » + 每页条数）
...
'settings.skillDesc': 'skill 使用排行：可切换两种记分规则，列表分页浏览。',
'settings.sessionDesc': '加载过 skill 的会话排行：去重 skill 数越多越靠前，并列按加载次数。',
'settings.manageDesc': '记分数据的导入、导出与概览。',
'settings.rankSession': '按会话去重',
'settings.rankLoads': '按全部加载',
...
'settings.skillDesc': 'Skill ranking: switch scoring rule, page through the list.',
...
'settings.rankSession': 'Per session',
'settings.rankLoads': 'Every load',
...
// 选项卡 / 翻页样式对齐插件市场（.tabs/.tab/.on/.subTabs/.pager）
const cssText = [
  ...
  // 二级翻页（两种排行规则）
  '.dshsb_subTabs{display:flex;align-items:flex-end;gap:2px;margin:-4px 0 2px}',
  '.dshsb_subTab{font:inherit;color:var(--dsw-alias-label-secondary);cursor:pointer;white-space:nowrap;background:0 0;border:none;border-bottom:2px solid transparent;padding:5px 10px;font-size:12px}',
  '.dshsb_subTab:hover{color:var(--dsw-alias-label-primary)}',
  '.dshsb_subTabOn{color:var(--dsw-alias-brand-primary);border-bottom-color:var(--dsw-alias-brand-primary);font-weight:600}',
...
// ── 记分板页面（三选项卡） ──────────────────────────────────────────────
/**
 * 按当前排行规则给 skill 榜排序：'loads' 用每次加载数，否则用会话去重数；并列按名称。
 *
 * @param skills - API 返回的 skill 行。
 * @param rankMode - 'session'（去重）| 'loads'（全部加载）。
 * @returns 排好序的新数组。
 */
function sortSkillRows(skills, rankMode) {
  return skills.slice().sort((a, b) => {
    const av = rankMode === 'loads' ? (a.loads || a.count || 0) : (a.count || 0)
    const bv = rankMode === 'loads' ? (b.loads || b.count || 0) : (b.count || 0)
...
skills: [], sessions: [], total: 0, totalLoads: 0, recorded: 0, updatedAt: null, dataFile: '',
  ...
  const [rankMode, setRankMode] = ReactRef.useState('session') // 'session'（去重）| 'loads'（全部加载）
...
fetch('/api/skill-scoreboard', { credentials: 'same-origin' })
  ...
  const skillRows = sortSkillRows(state.skills, rankMode)
...
return next
  ...
  const skillPanel = ReactRef.createElement(SkillPanel, { state, skillRows, skillSlice, rankMode, setRankMode, skillPage, skillPageCount, pageSize, goPage, changePageSize })
  ...
  const badgeValue = tab === 'session'
? sessionRows.length
: (rankMode === 'loads' ? (state.totalLoads || state.total) : (state.total || 0))
...
activePanel,
  ...
  /**
   * Skill 选项卡：二级翻页切换「按会话去重 / 按全部加载」两种排行，列表分页。
   */
  function SkillPanel({ state, skillRows, skillSlice, rankMode, setRankMode, skillPage, skillPageCount, pageSize, goPage, changePageSize }) {
// 由 ScoreboardPage 传入所需的 state/行数据/回调，本函数只负责渲染。
return ReactRef.createElement('div', { className: 'dshsb_panelWrap' },
  ReactRef.createElement('p', { className: 'dshsb_muted' }, tr('settings.skillDesc')),
  ReactRef.createElement('div', { className: 'dshsb_subTabs' },
    ReactRef.createElement('button', {
      type: 'button',
      className: rankMode === 'session' ? 'dshsb_subTab dshsb_subTabOn' : 'dshsb_subTab',
      onClick: () => { setRankMode('session'); goPage('skill', 1) },
    }, tr('settings.rankSession')),
    ReactRef.createElement('button', {
      type: 'button',
      className: rankMode === 'loads' ? 'dshsb_subTab dshsb_subTabOn' : 'dshsb_subTab',
      onClick: () => { setRankMode('loads'); goPage('skill', 1) },
    }, tr('settings.rankLoads')),
  ),
              ...
              skillSlice.map((row, i) => {
                ...
                return ReactRef.createElement('div', { className: 'dshsb_row', key: row.name, title: row.name },
                  ...
                  ReactRef.createElement('span', {
                    className: rankMode === 'session' ? 'dshsb_count dshsb_countHot' : 'dshsb_countDim',
                  }, String(count)),
                  ReactRef.createElement('span', {
                    className: rankMode === 'loads' ? 'dshsb_count dshsb_countHot' : 'dshsb_countDim',
                  }, String(loads)),
