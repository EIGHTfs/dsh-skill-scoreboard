...
function panelHeadRow(cols) {
  return ReactRef.createElement('li', { className: 'dshsb_headrow' },
    cols.map(([cls, key]) => ReactRef.createElement('span', { className: cls }, key ? tr(key) : '')),
  )
}
...
return next
  ...
  const badgeLabel = badgeFor(tab, state, sessionRows)
        ...
        ReactRef.createElement('input', {
          ...
          onChange: (ev) => {
            ...
            f.text().then((text) => fetch('/api/skill-scoreboard/import?merge=' + (merge ? 'true' : 'false'), {
              ...
              const payload = await res.json().catch((e) => { /* 响应非 JSON 时降级空数据 */ return {} })
              if (!res.ok || !payload.ok) throw payload.error ? new Error(payload.error) : httpError(res.status)
              setState((s) => ({ ...s, notice: tr('settings.importOk', { n: payload.recorded || 0 }) }))
              load()
