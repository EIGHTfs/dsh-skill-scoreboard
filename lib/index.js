...
function readPathFromArgs(args) {
  try {
    const obj = typeof args === 'string' ? JSON.parse(args) : args;
    const raw = obj?.file_path || obj?.filePath || '';
...
/** 会话短 id：去掉 session- 前缀后取前 8 位（与 client shortSessionId 一致，供标题兜底显示）。 */
 ...
 */
export function makeTitleResolver(ctx) {
  const cache = new Map();
  const TTL_MS = 60_000;
  ...
  async function resolveTitle(id) {
    ...
    let title = '';
    // ① 活跃会话列表快照（byId 的 key 兼容裸 id / session- 前缀两种形态）
    if (sessionsSvc && typeof sessionsSvc.list?.getSnapshot === 'function') {
      try {
        const byId = sessionsSvc.list.getSnapshot()?.byId || {};
        const row = byId[key] || byId['session-' + key] || byId[String(key).replace(/^session-/, '')];
        if (row && typeof row.displayTitle === 'string' && row.displayTitle) title = row.displayTitle;
      } catch { /* 忽略 */ }
    }
    // ② 历史会话：读持久化日志折叠标题
    if (!title && persistence && typeof persistence.inspect === 'function') {
      try {
        const loaded = await persistence.inspect(key);
        title = foldTitleFromEvents(loaded?.events);
        if (!title && loaded?.meta && typeof loaded.meta.cwd === 'string') {
          title = titleFromCwd(loaded.meta.cwd);
        }
      } catch { /* 读不到就继续降级 */ }
    }
    if (!title) title = shortIdOf(key); // 兜底：显示会话短 id（前 8 位裸 uuid）
    cache.set(key, { title, at: Date.now() });
    return title;
  }
  return resolveTitle;
}
...
export async function apply(ctx, config = {}) {
  ...
  registerWebRoutes(ctx, {
    file,
    titleResolver: makeTitleResolver(ctx),
 ...
 * @param file - 数据文件路径。
 */
function registerSnapshotRoute(webServer, file, titleResolver = null) {
  webServer.register({
    kind: 'exact',
    ...
    handler: async (req, res) => {
      ...
      try {
        const body = await buildSnapshot(file, titleResolver);
...
function registerWebRoutes(ctx, options) {
  ...
  ctx.inject(['webServer'], (wctx) => {
    ...
    if (!webServer) return;
    registerSnapshotRoute(webServer, file, options.titleResolver);
