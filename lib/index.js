 ...
 * 读记分文件 → 按次数降序 → Top N 行，每行经 skills 服务解析实际路径。
...
export async function buildScoreboardInjection({ file, topN = 25, skillsSvc = null }) {
  ...
  const rows = Object.entries(recs)
    .map(([n, rec]) => ({ name: n, count: rec?.count || 0, lastUsedAt: rec?.lastUsedAt || null }))
  ...
  const lines = [
    '【dsh-skill-scoreboard 注入：skill 使用记分榜】AI 会话实际加载过的 skill 统计（按会话去重、跨会话累计；数据源 data/skill-usage.json）。供参考：常用/已加载过的 skill 及其实际文件路径，避免无谓重复加载、便于定位 skill 文件：',
    '',
    `- 共 ${rows.length} 个 skill、累计 ${totalCount} 次（Top ${top.length}）：`,
  ];
  for (let i = 0; i < top.length; i++) {
    const row = top[i];
    const path = paths[i] || '';
    lines.push(`  ${i + 1}. ${row.name} ×${row.count}（最近 ${fmtShortTime(row.lastUsedAt)}）${path ? ` → ${path}` : ''}`);
  }
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        ...
        const snapshot = () => {
          ...
          return { ok: true, total, recorded: skills.length, updatedAt: current.updatedAt || null, skills };
        };
        if (method === 'GET') {
