...
/** 校验导入体：必须是 { version, skills: { name: { count, lastUsedAt?, sessions?, callIds? } } } */
export function normalizeScoreboardPayload(raw) {
  const obj = typeof raw === 'string' ? JSON.parse(raw) : raw;
  if (!obj || typeof obj !== 'object' || Array.isArray(obj)) {
    throw new Error('记分文件必须是 JSON 对象');
  }
  const skillsIn = obj.skills && typeof obj.skills === 'object' && !Array.isArray(obj.skills) ? obj.skills : null;
  if (!skillsIn) throw new Error('记分文件缺少 skills 对象');
  const skills = {};
  for (const [name, rec] of Object.entries(skillsIn)) {
    const n = String(name || '').trim();
    if (!n) continue;
    const count = Number(rec?.count) || 0;
    skills[n] = {
      count,
      loads: Number(rec?.loads) || count,
      lastUsedAt: rec?.lastUsedAt || null,
      sessions: Array.isArray(rec?.sessions) ? rec.sessions.map(String) : [],
      callIds: Array.isArray(rec?.callIds) ? rec.callIds.map(String) : [],
    };
  }
  return {
    version: 1,
...
/** merge=true 时按 skill 名累加次数、合并会话/callId；false 则整表替换。 */
export function mergeScoreboard(current, incoming) {
  const base = current?.skills && typeof current.skills === 'object' ? current.skills : {};
  const extra = incoming?.skills && typeof incoming.skills === 'object' ? incoming.skills : {};
  const skills = { ...base };
  for (const [name, rec] of Object.entries(extra)) {
    const prev = skills[name];
    if (!prev) {
      skills[name] = { ...rec };
      continue;
    }
    const sessions = [...new Set([...(prev.sessions || []), ...(rec.sessions || [])])];
    const callIds = [...new Set([...(prev.callIds || []), ...(rec.callIds || [])])];
    const lastA = prev.lastUsedAt || '';
    const lastB = rec.lastUsedAt || '';
    const prevLoads = Number(prev.loads) || Number(prev.count) || 0;
    const recLoads = Number(rec.loads) || Number(rec.count) || 0;
    skills[name] = {
      count: (Number(prev.count) || 0) + (Number(rec.count) || 0),
      loads: prevLoads + recLoads,
      lastUsedAt: lastB > lastA ? rec.lastUsedAt : prev.lastUsedAt,
      sessions,
      callIds,
    };
  }
...
export async function buildScoreboardInjection({ file, topN = 25, skillsSvc = null }) {
  ...
  const lines = [
    '【dsh-skill-scoreboard 注入：skill 使用记分榜】AI 会话实际加载过的 skill 统计。count=按会话去重（同会话同一 skill 只计 1），loads=每次成功加载都计。数据源 data/skill-usage.json。供参考：常用/已加载过的 skill 及其实际文件路径，避免无谓重复加载、便于定位 skill 文件：',
    '',
  ...
  lines.push('', '（完整榜单见 设置 → 侧边栏 → Skill 记分板；页面可切换两种记分规则）');
  return lines.join('\n');
}
...
export async function apply(ctx, config = {}) {
  ...
  ctx.on('tools/result', (exec, result) => {
    ...
    writeChain = writeChain.then(async () => {
      ...
      const target = rec ?? { count: 0, loads: 0, lastUsedAt: null, sessions: [], callIds: [] };
      if (typeof target.loads !== 'number') target.loads = target.count || 0;
      target.loads += 1;
      const alreadySession = !!(rec && rec.sessions?.includes(sessionId));
      if (!alreadySession) {
        target.count += 1;
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        ...
        const snapshot = () => {
          ...
          const totalLoads = skills.reduce((s, x) => s + x.loads, 0);
          return { ok: true, total, totalLoads, recorded: skills.length, updatedAt: current.updatedAt || null, skills };
        };
        if (method === 'GET') {
          try { json(200, snapshot()); } catch (e) { json(500, { ok: false, error: String(e?.message ?? e) }); }
          return;
        }
    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          data = next;
          const recorded = Object.keys(next.skills || {}).length;
          json(200, { ok: true, merge, recorded, updatedAt: next.updatedAt });
        } catch (e) {
          json(400, { ok: false, error: String(e?.message ?? e) });
        }
      },
    });
    console.log('[skill-scoreboard] ✅ 已注册 GET /api/skill-scoreboard + export/import');
  });
}