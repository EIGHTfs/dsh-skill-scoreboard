...
// 2026-09-13：client.js 内 require('./i18n/*.json')（Function 执行无模块路径基准），
// 用 createRequire(import.meta.url) 提供以本测试文件为基准的真实 require 兜底。
...
const mod = entry.factory((id) => (id === 'react' ? React : requireFromTest(join(__dirname, '..', 'lib', id.slice(2)))))
...
// 回归：v1.8.2 曾发生「loading 时点管理 tab，整个设置面板消失」。
// 根因是 ManagePanel 引用了其作用域外的 skillRows（ManagePanel 是顶层函数而非
// ScoreboardPage 内部函数），loading 态 state.recorded=0（falsy）会继续求值
// skillRows.length → ReferenceError → settings.section 无错误边界 → 整棵面板卸载。
// 数据加载完后 recorded 为 truthy、|| 短路，才不触发——所以只有 loading 时点才炸。
// 这里用源码级断言锁定：ManagePanel 函数体不得引用作用域外的 skillRows。
const clientSrc = readFileSync(join(__dirname, '..', 'lib', 'client.js'), 'utf8')
const manageFnSrc = clientSrc.slice(
  clientSrc.indexOf('function ManagePanel('),
  clientSrc.indexOf('function createScoreboardUi('),
)
ok(!/\bskillRows\b/.test(manageFnSrc), '回归防护：ManagePanel 不引用作用域外的 skillRows（loading 切管理不再崩面板）')
