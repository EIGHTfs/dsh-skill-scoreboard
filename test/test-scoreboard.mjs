...
try {
  ...
  ok((await resolver.resolve('session-abc123')) === 'abc123', '无服务时标题回退会话短 id');
  ok((await resolver.resolve('abc123')) === 'abc123', '裸 id 短 id 兜底不变');
  ...
  ok((await resolverLive.resolve('session-live1')) === '活性标题', '活跃快照 displayTitle 生效');
  ok((await resolverLive.resolve('live1')) === '活性标题', '裸 id 也能命中活跃快照（补 session- 前缀）');
  ...
  ok((await resolverPersist.resolve('cold1')) === '持久标题-cold1', '历史会话从持久化日志折叠 session/title');
  ok((await resolverPersist.resolve('cold1')) === '持久标题-cold1', '第二次 resolve 命中内存缓存（不再调 inspect）');

  // v1.8.2：落盘缓存——解析结果写入缓存文件，新 resolver 从缓存直接命中（不再 inspect）
  const titleCacheFile = join(root, 'session-titles.json');
  const resolverCached = makeTitleResolver({
    get: (n) => (n === 'sessionPersistence' ? {
      inspect: async (id) => { throw new Error('不应再调 inspect（缓存命中）') },
    } : null),
  }, titleCacheFile);
  // 先落一份缓存（模拟之前解析过 cold-cached 会话）
  writeFileSync(titleCacheFile, JSON.stringify({ 'cold-cached-1': '磁盘缓存标题' }), 'utf8');
  const resolverFromDisk = makeTitleResolver({ get: () => null }, titleCacheFile);
  ok((await resolverFromDisk.resolve('cold-cached-1')) === '磁盘缓存标题', '落盘缓存命中：不从日志 inspect 直接出标题');
  // warm 预热：已知 id 列表全部进缓存
  const resolverWarm = makeTitleResolver({
    get: (n) => (n === 'sessionPersistence' ? {
      inspect: async (id) => ({ events: [{ type: 'session/title', data: { title: '预热标题-' + id } }], meta: {} }),
    } : null),
  }, join(root, 'session-titles-warm.json'));
  await resolverWarm.warm(['w1', 'w2']);
  ok((await resolverWarm.resolve('w1')) === '预热标题-w1' && (await resolverWarm.resolve('w2')) === '预热标题-w2', 'warm 预热后直接命中内存缓存');
  const warmCache = JSON.parse(readFileSync(join(root, 'session-titles-warm.json'), 'utf8'));
  ok(warmCache['w1'] === '预热标题-w1' && warmCache['w2'] === '预热标题-w2', 'warm 结果已落盘');
